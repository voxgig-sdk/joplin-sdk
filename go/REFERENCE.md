# Joplin Golang SDK Reference

Complete API reference for the Joplin Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *JoplinSDK`
...
```go
result, err := client.Note(nil).Load(map[string]any{"id": "note_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
...
```go
result, err := client.Note(nil).Remove(map[string]any{"id": "note_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
...
```go
result, err := client.Tag(nil).Update(map[string]any{
    "id": "tag_id",
    // Fields to update
}, nil)
if err != nil {
