...
//    only - a typed JoplinContext there would make every ctx
...
final class JoplinOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new JoplinContext($clean, null);
    ...
    public static function err_from_map($m): ?JoplinError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new JoplinError(is_string($code) ? $code : '', $msg);
    }
