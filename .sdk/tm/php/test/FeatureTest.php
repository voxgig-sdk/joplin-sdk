...
class FtCtrl extends JoplinControl
{
...
class FtClient
{
    ...
    private JoplinUtility $utility;

    public function __construct(JoplinUtility $utility, array $options)
    {
    ...
    public function get_utility(): JoplinUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JoplinContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new JoplinUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (JoplinContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JoplinConfig::make_feature($name);
    ...
    private static function buildUrl(JoplinSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JoplinError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JoplinResponse($fetched);
        }
    ...
    private function fail(JoplinContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JoplinContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
