...
module JoplinUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = JoplinContext.new({}, nil)
    end
    op = ctx.op || JoplinOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || JoplinResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(JoplinError) ? err.msg : err.to_s
    ...
    sdk_err = JoplinError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(JoplinError)
