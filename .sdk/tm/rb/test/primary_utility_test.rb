...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = JoplinOmni.make_runner(TEST_JSON_FILE, JoplinSDK.test(nil, nil))
  ...
  def test_feature_hook_basic
    hook_client = JoplinSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = JoplinSDK.test(nil, nil)
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = JoplinSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = JoplinSDK.test(nil, setup_opts)
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : JoplinOmni.errify(err)
    end
  ...
  def test_make_url_basic
    subject = lambda do |ctx|
      ctx.result = JoplinResult.new({}) unless ctx.result
  ...
  def test_prepare_auth_basic
    ...
    auth_client = JoplinSDK.test(nil, setup_opts)
