...
//
// NewJoplinSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.JoplinSDK {
	res, _ := kase["res"].([]any)
...
func fcCandidates(client *sdk.JoplinSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.JoplinSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.JoplinSDK, name string) any {
