// Command Joplin-cli is a CLI + REPL for the Joplin SDK,
...
const entitiesHelp = "folder note tag"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      JOPLIN_APIKEY (api key), JOPLIN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: JOPLIN_APIKEY carries the API key and
	// JOPLIN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("JOPLIN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("JOPLIN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewJoplinSDK(opts)
