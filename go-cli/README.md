...
boru-driven command-line client **and** interactive REPL for the Joplin
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/joplin-cli)
make build
...
./joplin-cli list folder
./joplin-cli load 1 folder            # {id:1} shorthand
./joplin-cli load '{id:1}' folder       # explicit match map
./joplin-cli update '{name:"x"}' folder
./joplin-cli list note

# 5. Override the API base URL for a single call
JOPLIN_BASE=https://api.example.com ./joplin-cli list folder
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/joplin-cli
   ```
...
```sh
./joplin-cli load 1 folder          # scalar shorthand for {id:1}
./joplin-cli load '{id:1}' folder     # explicit match map
```
...
```sh
./joplin-cli update '{id:1,name:"new"}' folder
```
...
Run with no arguments to open a REPL (prompt `joplin>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `folder`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/joplin-cli`. |
...
(`./joplin-cli <expr>`) and interactively (the REPL), and expressions compose the
