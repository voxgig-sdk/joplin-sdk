...
class JoplinTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JoplinTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JoplinTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JoplinTestRunner.getenv("JOPLIN_TEST_LIVE")
        override = JoplinTestRunner.getenv("JOPLIN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JoplinTestRunner.getenv(key)
        ...
        explain = JoplinTestRunner.getenv("JOPLIN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["JOPLIN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if JoplinTestRunner._test_control is not None:
            return JoplinTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JoplinTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JoplinTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JoplinTestRunner.load_test_control()
...
def load_env_local():
    JoplinTestRunner.load_env_local()


def env_override(m):
    return JoplinTestRunner.env_override(m)


def entity_data(v):
    return JoplinTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JoplinTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JoplinTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JoplinTestRunner.load_test_control()


def live_client_options():
    return JoplinTestRunner.live_client_options()


def live_delay_ms():
    return JoplinTestRunner.live_delay_ms()
