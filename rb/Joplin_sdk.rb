...
class JoplinSDK
  ...
  def initialize(options = {})
    ...
    utility = JoplinUtility.new
    @_utility = utility

    config = JoplinConfig.shared_config
    ...
    feature_opts = JoplinHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = JoplinHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, JoplinFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = JoplinHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = JoplinHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = JoplinHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    allow_op = VoxgigStruct.getpath(@options, "allow.op")
    {
      ...
      "err" => JoplinError.new(
        "#{op}_allow",
        "JoplinSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = JoplinHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = JoplinError.new(
        "graphql_error", "JoplinSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Folder.list / client.Folder.load({ "id" => ... })
  def Folder(data = nil)
    require_relative 'entity/folder_entity'
    FolderEntity.new(self, data)
  end


  # Canonical facade: client.Note.list / client.Note.load({ "id" => ... })
  def Note(data = nil)
    require_relative 'entity/note_entity'
    NoteEntity.new(self, data)
  end
