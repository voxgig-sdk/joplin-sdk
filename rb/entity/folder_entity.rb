...
class FolderEntity
  ...
  def make
    opts = @_entopts.dup
    FolderEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JoplinHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Folder, Hash] the current Folder data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JoplinHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Folder fields)
  def match_get
  ...
  #
  # @param reqmatch [FolderLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Folder.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Folder, Hash] the loaded Folder; raises JoplinError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JoplinHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [FolderListMatch, Hash, nil] match filter (any subset of
  #   Folder fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Folder>, Array] the matching Folder items; raises JoplinError on failure
  ...
  # Create a new Folder.
  #
  # @param reqdata [FolderCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Folder, Hash] the created Folder; raises JoplinError on failure
  ...
  #
  # @param reqdata [FolderUpdateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Folder, Hash] the updated Folder; raises JoplinError on failure
  ...
  #
  # @param reqmatch [FolderRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [Folder, Hash] the removed Folder; raises JoplinError on failure
