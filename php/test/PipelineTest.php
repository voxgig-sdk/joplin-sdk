...
{
    private static function utility(): JoplinUtility
    {
        return new JoplinUtility();
    }
    ...
    private static function ctx(array $over = []): JoplinContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new JoplinContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new JoplinOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof JoplinError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new JoplinOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = JoplinMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new JoplinOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = JoplinMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new JoplinOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = JoplinMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JoplinSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = JoplinMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = JoplinMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new JoplinSpec(['step' => 's']),
            'response' => new JoplinResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new JoplinResult([]),
        ]);
        [, $err] = JoplinMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new JoplinSpec(['step' => 's']),
            'response' => new JoplinResponse(self::resp(200, ['v' => 1])),
            'result' => new JoplinResult([]),
        ]);
        [, $err] = JoplinMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new JoplinSpec(['step' => 's']),
            'response' => new JoplinResponse(self::resp(200, ['v' => 2])),
            'result' => new JoplinResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        JoplinMakeResponse::call($ctx);
    ...
    {
        $preset = new JoplinResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new JoplinSpec([]),
            'response' => new JoplinResponse([]),
            'result' => new JoplinResult([]),
        ...
        [$got, $err] = JoplinMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new JoplinResult([])]);
        [, $err] = JoplinMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new JoplinSpec([]), 'result' => null]);
        [, $err] = JoplinMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JoplinSpec(['step' => 's']),
            'result' => new JoplinResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new JoplinOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JoplinMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JoplinSpec(['step' => 's']),
            'result' => new JoplinResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new JoplinOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JoplinMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JoplinResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new JoplinSpec([]),
            'result' => new JoplinResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = JoplinMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JoplinMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new JoplinError('boom', 'boom');
        $utility->fetcher = function (JoplinContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JoplinSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = JoplinMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (JoplinContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = JoplinMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(JoplinResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (JoplinContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JoplinSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = JoplinMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new JoplinResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new JoplinSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = JoplinMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = JoplinMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = JoplinMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new JoplinResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, JoplinDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new JoplinResult(['ok' => false])]);
        $this->expectException(JoplinError::class);
        JoplinDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new JoplinResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', JoplinMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new JoplinBaseFeature();
        $b = new JoplinBaseFeature();
        JoplinFeatureAdd::call($ctx, $a);
        JoplinFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): JoplinBaseFeature
    {
        $f = new JoplinBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        JoplinFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): JoplinContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new JoplinSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JoplinPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new JoplinResponse(['headers' => null]),
            'result' => new JoplinResult([]),
        ]);
        JoplinResultHeaders::call($ctx);
