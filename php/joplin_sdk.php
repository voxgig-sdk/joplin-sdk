...
class JoplinSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = JoplinHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = JoplinHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!JoplinFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, JoplinFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return JoplinUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = JoplinHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = JoplinHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = JoplinHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = JoplinHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new JoplinError("graphql_error",
                "JoplinSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
