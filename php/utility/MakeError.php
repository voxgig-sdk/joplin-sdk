...
class JoplinMakeError
{
    public static function call(?JoplinContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new JoplinContext([], null);
        }
        $op = $ctx->op ?? new JoplinOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new JoplinResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof JoplinError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new JoplinError('', $msg, $ctx);
