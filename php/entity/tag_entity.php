...
class TagEntity
{
    ...
    {
        if ($args) {
            $this->_data = JoplinHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = JoplinHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws JoplinError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = JoplinHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws JoplinError on failure (item-5 convention).
     */
     ...
     *
     * @param TagUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed TagUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Tag|array The updated Tag as an assoc-array at the
     *   SDK boundary; throws JoplinError on failure (item-5 convention).
     */
