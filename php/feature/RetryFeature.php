...
class JoplinRetryFeature extends JoplinBaseFeature
{
    ...
    public function init(JoplinContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JoplinContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JoplinContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JoplinContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
