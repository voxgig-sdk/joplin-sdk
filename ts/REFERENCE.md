# Joplin TypeScript SDK Reference

Complete API reference for the Joplin TypeScript SDK.
...
#### `JoplinSDK.test(testopts?, sdkopts?)`
...
**Returns:** `JoplinSDK` instance in test mode.
...
| Action | Route | Call |
| --- | --- | --- |
| `note` | `/folders/{id}/notes` | `client.Folder().list({ $action: 'note', ... })` |
...
Return the parent `JoplinSDK` instance.
...
| Action | Route | Call |
| --- | --- | --- |
| `tag` | `/notes/{id}/tags` | `client.Note().list({ $action: 'tag', ... })` |

An action returns that action's OWN response, which is not necessarily a
Note record — check the API definition for its shape.

```ts
const result = await client.Note().list({
  $action: 'tag',
  /* ...the action's own arguments */
})
```
...
```ts
const result = await client.Note().load({ id: 'note_id' })
```
...
```ts
const result = await client.Note().remove({ id: 'note_id' })
```
...
| Action | Route | Call |
| --- | --- | --- |
| `note` | `/tags/{id}/notes` | `client.Tag().list({ $action: 'note', ... })` |
