...
const TEST_SEED = { "entity": { "folder": { "test01": { "id": "test01" } }, "note": { "test01": { "id": "test01" } }, "tag": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+JoplinSDK\b/.test(code) ||
        /\bJoplinSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+JoplinSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/JoplinSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
