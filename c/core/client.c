// JoplinSDK client (generated — mirrors the rust Main fragment).
...
JoplinSDK* joplin_sdk_new(voxgig_value* options) {
  JoplinSDK* sdk = (JoplinSDK*)calloc(1, sizeof(JoplinSDK));
...
static bool sdk_op_allowed(JoplinSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(JoplinSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
  const char* allow = voxgig_is_string(allow_op) ? voxgig_as_string(allow_op) : "";
  char msg[512];
  snprintf(msg, sizeof(msg),
    "JoplinSDK: %s: operation not allowed by"
...
static voxgig_value* sdk_raw_request(
  JoplinSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(JoplinSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "JoplinSDK: graphql: %s", m);
...
// Folder entity bound to this client.
Entity* joplin_folder(JoplinSDK* client, voxgig_value* entopts) {
  return folder_entity_new(client, entopts);
}

// Note entity bound to this client.
Entity* joplin_note(JoplinSDK* client, voxgig_value* entopts) {
  return note_entity_new(client, entopts);
}
...
JoplinSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  JoplinSDK* sdk = joplin_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
