// Generated direct-call test for the note entity (mirrors the
...
static voxgig_value* note_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static JoplinSDK* note_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(note_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    JoplinSDK* sdk = note_direct_setup(mockres);
  ...
  TEST_SUMMARY("joplin_note_direct");
}
