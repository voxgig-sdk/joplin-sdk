// Generated direct-call test for the folder entity (mirrors the
...
static voxgig_value* folder_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static JoplinSDK* folder_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(folder_mock, mockres)));
  return joplin_sdk_new(opts);
}
...
int main(void) {
  ...
  // LIST
  {
    ...
    JoplinSDK* sdk = folder_direct_setup(mockres);
  ...
  TEST_SUMMARY("joplin_folder_direct");
}
