// Folder entity client (generated — mirrors the rust Entity fragment).
...
typedef void (*folder_postdone_fn)(folder_entity* self, Context* ctx);

// Forward declarations.
static const EntityVT folder_VT;
static const char* folder_get_name(Entity* e);
static Entity* folder_make(Entity* e);
static voxgig_value* folder_data(Entity* e, voxgig_value* args);
static voxgig_value* folder_matchv(Entity* e, voxgig_value* args);
// Ops resolve to the ENTITY (`list` to a NULL-terminated array of them).
static Entity* folder_load(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err);
static Entity** folder_list(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err);
static Entity* folder_create(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err);
static Entity* folder_update(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err);
static Entity* folder_remove(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err);
static void folder_mark_deleted(Entity* e);
static bool folder_deleted(Entity* e);

static Context* folder_ent_ctx(folder_entity* self) {
  return self->entctx;
}
...
Entity* folder_entity_new(JoplinSDK* client, voxgig_value* entopts) {
  ...
  folder_entity* self = (folder_entity*)calloc(1, sizeof(folder_entity));
  self->base.vt = &folder_VT;
  self->name = strdup("folder");
  self->client = client;
...
static voxgig_value* folder_run_op(folder_entity* self, Context* ctx,
                                    folder_postdone_fn post_done, PNError** err) {
...
voxgig_value* folder_stream(Entity* e, const char* action, voxgig_value* args,
                             voxgig_value* callopts, PNError** err) {
  folder_entity* self = (folder_entity*)e;
  *err = NULL;
  ...
  Context* ctx = make_context_util(cs, folder_ent_ctx(self));
...
static Entity* folder_make(Entity* e) {
  ...
  return folder_entity_new(self->client, opts);
}
...
static voxgig_value* folder_data(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(folder_ent_ctx(self), "SetData");
  }
  feature_hook_util(folder_ent_ctx(self), "GetData");
...
static voxgig_value* folder_matchv(Entity* e, voxgig_value* args) {
  ...
  if (args && !v_is_noval(args) && !v_is_null(args)) {
    ...
    feature_hook_util(folder_ent_ctx(self), "SetMatch");
  }
  feature_hook_util(folder_ent_ctx(self), "GetMatch");
  return voxgig_clone(self->mtch);
}


static void folder_load_postdone(folder_entity* self, Context* ctx) {
...
static Entity* folder_load(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err) {
  ...
  cs.reqmatch = reqmatch;
  Context* ctx = make_context_util(cs, folder_ent_ctx(self));
  folder_run_op(self, ctx, folder_load_postdone, err);
  if (*err) return NULL;
...
static void folder_list_postdone(folder_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    if (voxgig_is_map(resmatch)) self->mtch = resmatch;
...
static Entity** folder_list(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err) {
  ...
  cs.reqmatch = reqmatch;
  Context* ctx = make_context_util(cs, folder_ent_ctx(self));
  voxgig_value* out = folder_run_op(self, ctx, folder_list_postdone, err);
  if (*err) return NULL;
...
static void folder_create_postdone(folder_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resdata = result->resdata;
    if (!v_is_noval(resdata) && !v_is_null(resdata)) {
...
static Entity* folder_create(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err) {
  ...
  cs.reqdata = reqdata;
  Context* ctx = make_context_util(cs, folder_ent_ctx(self));
  folder_run_op(self, ctx, folder_create_postdone, err);
  if (*err) return NULL;
...
static void folder_update_postdone(folder_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    voxgig_value* resdata = result->resdata;
...
static Entity* folder_update(Entity* e, voxgig_value* reqdata, voxgig_value* ctrl, PNError** err) {
  ...
  cs.reqdata = reqdata;
  Context* ctx = make_context_util(cs, folder_ent_ctx(self));
  folder_run_op(self, ctx, folder_update_postdone, err);
  if (*err) return NULL;
...
static void folder_remove_postdone(folder_entity* self, Context* ctx) {
  SdkResult* result = ctx->result;
  if (result) {
    voxgig_value* resmatch = result->resmatch;
    voxgig_value* resdata = result->resdata;
...
static Entity* folder_remove(Entity* e, voxgig_value* reqmatch, voxgig_value* ctrl, PNError** err) {
  ...
  cs.reqmatch = reqmatch;
  Context* ctx = make_context_util(cs, folder_ent_ctx(self));
  folder_run_op(self, ctx, folder_remove_postdone, err);
  if (*err) return NULL;
...
// live record.
static void folder_mark_deleted(Entity* e) {
  ((folder_entity*)e)->deleted = true;
}

static bool folder_deleted(Entity* e) {
  return ((folder_entity*)e)->deleted;
}
